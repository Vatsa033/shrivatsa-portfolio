document.addEventListener("DOMContentLoaded",()=>{
  ...
  const designs={
    ...
    baggage:{title:"Open Baggage Cart",kicker:"GSE · MECHANICAL DESIGN",desc:"Two rendered views of the open baggage cart design.",images:["assets/baggage-cart-01.png","assets/baggage-cart-02.png"]}
  };
  const modal=document.querySelector(".design-modal");
  if(modal){let current=0,items=[];const image=modal.querySelector("#modal-image"),title=modal.querySelector("#modal-title"),kicker=modal.querySelector("#modal-kicker"),desc=modal.querySelector("#modal-desc"),count=modal.querySelector("#modal-count"),thumbs=modal.querySelector("#modal-thumbs");
    const render=()=>{image.style.opacity=0;setTimeout(()=>{image.src=items[current];image.alt=title.textContent+" — view "+(current+1);image.style.opacity=1;count.textContent=String(current+1).padStart(2,"0")+" / "+String(items.length).padStart(2,"0");if(thumbs)[...thumbs.children].forEach((b,k)=>b.classList.toggle("active",k===current))},120)};
    const open=key=>{const d=designs[key];if(!d)return;items=d.images;current=0;title.textContent=d.title;kicker.textContent=d.kicker;desc.textContent=d.desc;if(thumbs){thumbs.innerHTML="";items.forEach((src,j)=>{const b=document.createElement("button");b.type="button";b.className="modal-thumb";b.setAttribute("aria-label","View "+(j+1));const im=document.createElement("img");im.src=src;im.alt="";b.appendChild(im);b.addEventListener("click",()=>{current=j;render()});thumbs.appendChild(b)})}modal.classList.add("open");modal.setAttribute("aria-hidden","false");render();document.body.style.overflow="hidden"};
